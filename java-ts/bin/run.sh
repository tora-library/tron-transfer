...
APP_HOME=$CUR_PATH/bin

#需要启动的Java主程序（main方法类）
APP_MAINCLASS=org.ts.TsApiApplication
...
if [ -n "$JAVA_HOME" ] ; then
    ...
    JAVACMD="java"
    which java >/dev/null 2>&1 || echo "ERROR: JAVA_HOME is not set and no 'java' command could be found in your PATH. Please set the JAVA_HOME variable in your environment to match the location of your Java installation." && exit 1
fi
...
start() {
   ...
   if [ $psid -ne 0 ]; then
      ...
      nohup $JAVACMD $JAVA_OPTS -classpath $CLASSPATH $APP_MAINCLASS >/dev/null 2>nohup.out &
      checkpid
      if [ $psid -ne 0 ]; then
         echo "(pid=$psid) [OK]"
      else
